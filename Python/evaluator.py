...
def count_pos_neg(libfm_test: Path):
    ...
    for i, line in enumerate(test_data):
        ...
        try:
            ...
            raise ValueError(f'The test data is not in the correct format. Value is not int {type(line[0])}.')
        ...
        if i_int == 0:
            ...
            raise ValueError(f'The test data is not in the correct format. Value < 0 : {i_int}.')
    #print(f"lengths - y_pred_pos: {len(y_pred_pos)}, y_pred_neg: {len(y_pred_neg)}")
    return y_pred_pos, y_pred_neg

def positive_in_top_50_prob(x):
    if x <= 49:
        return 1.0
    elif x > 100000:
        x = 100000
    P = (math.comb(1,1)*math.comb((x+1)-1,50-1))/math.comb(x+1,50)
    return P

def evaluate_file(file_path: Path):
    # Load test data
    libfm_test = Path(f'{config["PATHS"]["libfm_path"]}/{config["FILENAMES"]["libfm_test"]}')
    test_data = ut.load_file(libfm_test, logging=False)
...
def calculate_results(command: str):
    ...
    prob = positive_in_top_50_prob(n_neg)
