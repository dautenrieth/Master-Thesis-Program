...
#main_command = 'libFM -task r -train train -test test -dim "1,1,8" -iter 100 -method mcmc -validation valid -out test.pred -verbosity 1 -cache_size 1000000000 -meta groups.groups'
...
def main():
    ...
    for main_command in commands:
        ...
        if ut.check_folderstructure() is False:
            logger.error("Folder structure is not correct. Please check the config.ini file.\
        ...
        if config['PARTACTIVATED'].getboolean('libfm_run'):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                commands = []
                for i in range(n):
                    command = main_command.replace('-out test.pred', f'-out predictions/test{i}.pred')
