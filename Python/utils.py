...
def data_config_log():
    ...
    for key in config['DATASTRUCTURE']:
        if len(key) > max_key_length and not "_abbreviation" in key: # remove key if it has _abbreviation in it
    ...
    for key in config['DATASTRUCTURE']:
        if key != 'Grouping' and not "_abbreviation" in key:
            ...
            ckey = key.replace('_', ' ')
            ckey = ckey.title()
            config_string += f"{ckey.ljust(max_key_length)}: {config['DATASTRUCTURE'][key]}\n"
    config_string += f'\n{"-"*(_length)}\n'
    # Add creation date
    config_string += f"{'Created'.ljust(max_key_length)}: {dt_string}\n"
    # Add version
    config_string += f"{'Version'.ljust(max_key_length)}: {config['VERSION']['version']}\n"
    config_string += f'\n{"-"*(_length)}\n'

    # Write config to file named data_config.txt
    filename = Path(config['PATHS']['libfm_path'], config["FILENAMES"]["data_config_file"])
    with open(filename, 'w') as f:
...
def check_folderstructure():
    ...
    if not Path(config['PATHS']['libfm_path']).exists():
        logger.error(f"Folder {config['PATHS']['libfm_path']} does not exist.\
    ...
    if not Path(config['PATHS']['libfm_path'], 'libfm.exe').exists():
        logger.error(f"File {config['PATHS']['libfm_path']}libfm.exe does not exist.\
    ...
    if not Path(config['PATHS']['libfm_path'], 'transpose.exe').exists():
        logger.error(f"File {config['PATHS']['libfm_path']}transpose.exe does not exist.\
            Please download libfm from https://www.libfm.org/ and place the files in the libfm folder!")
        return False
    if not Path(config['PATHS']['libfm_path'], 'convert.exe').exists():
        logger.error(f"File {config['PATHS']['libfm_path']}convert.exe does not exist.\
            Please download libfm from https://www.libfm.org/ and place the files in the libfm folder!")
        return False

    # Check if neighborhood_folder exists
    if not Path(config['FOLDERNAMES']['neighborhood_folder']).exists():
        logger.error(f"Folder {config['FOLDERNAMES']['neighborhood_folder']} does not exist.\
...
def data_folders_naming()->Path:
    ...
    if config['NUMBERINSTANCES'].getboolean('all'):
        ...
        folder_path = Path(f"{config['PATHS']['data_path']}/{graph_name}_{parts}_{config['NUMBERINSTANCES']['number']}")
    return folder_path
...
def copy_data_files():
    ...
    for filename in filenames:
        ...
        if not file_path.exists():
            logger.error(f"File {filename} seems not be generated. Please check if this is correct!")
        else:
...
def filelist_data():
    ...
    # Add groups to keys
    filenames.append('groups.groups')
    # Add config text file to keys
    filenames.append('data_config.txt')
